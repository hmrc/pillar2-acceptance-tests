...
class UPENoIDFLowPageSpec extends BaseSpec with Matchers {
  ...
  Feature("UPE NO ID journey") {
    ...
    Scenario("1 - UPE No Id journey navigation to check your answers page and verify if data is pre populated", Wip) {
      ...
      givenIAmOnXPage("Give feedback") // auto-chosen (score=1.00, CommonStepsSteps.scala)
    ...
    Scenario("2 - Verify task list status", Wip) {
    ...
    Scenario("5 - Status update for Enter ultimate parent’s details task", Wip) {
      Given("Organisation User logs in to subscribe for Pillar2")
      givenXLogsInToSubscribeForPillar2("Organisation User") // auto-chosen (score=1.00, CommonStepsSteps.scala)
