...
class BTNUnderEnquirySpec extends BaseSpec {

  Feature("Below threshold notification under enquiry journeys") {

    Scenario("1 - Org user navigates to Under Enquiry Page when BTN submission is attempted with enquiry flag true", AcceptanceTests) {
    ...
    Scenario("2 - Agent user navigates to Under Enquiry Page when BTN submission is attempted with enquiry flag true", AcceptanceTests) {
    ...
    Scenario("3 - Org user navigates to Under Enquiry Page where BTN submission is attempted but org user changes mind and clicks link to navigate back to dashboard", AcceptanceTests) {
      ...
      login(
        ...
        page = "dashboard",
        enrolment = Some(
          Enrolment("HMRC-PILLAR2-ORG", "PLRID", "XEPLR9999999995")
        )
      )
      ...
      Then("The user is on the BTN under enquiry page and decides not to continue with the submission and clicks the link to return to dashboard")
      BtnUnderEnquiryPage.onPage()
      BtnUnderEnquiryPage.clickLinkReturnToHomepage()
  ...
  Scenario("4 - Agent user navigates to Under Enquiry Page where BTN submission is attempted but agent user changes mind and clicks link to navigate back to dashboard", AcceptanceTests) {
    Given("Agent User logs in with existing entity group")
    login(
      userType = "Agent",
      page = "asa",
    ...
    And("The agent returns to the accounting period and selects previous accounting period and reaches BTN under enquiry page")
    BtnAccountingPage.clickBackLink()
    BtnMultipleAccountingPage.selectRadioPreviousAccountingPeriod()
    ...
    Then("The agent is returned to the dashboard")
    DashboardPage.onPage()
