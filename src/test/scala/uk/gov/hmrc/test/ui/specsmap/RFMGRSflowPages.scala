...
import uk.gov.hmrc.test.ui.cucumber.stepdefs.EligibilityQuestionSteps.{thenIEnterAddressAs, thenIShouldBeOnX, thenIShouldBeRedirectToX, thenIShouldNavigateToX}
...
import uk.gov.hmrc.test.ui.cucumber.stepdefs.UPESteps.{andIEnterXAsX, andIRegisteredSuccessfullyWithX, andISelectOptionXAndContinueToGRSPage, andISelectOptionXAndContinueToNamePage, andRegistrationIsUnsuccessfulWithXError}

class RFMGRSflowPages extends AnyFeatureSpec with Matchers {
  ...
  Feature("RFM Ultimate Parent Entity and New nominated Filling Member GRS journey") {
    ...
    Scenario("2 - Verify that New NFM can replace existing FM through GRS registration flow on the Pillar 2 account and validations") {
      ...
      And("I provide RFM contact name as RFM test contact change")
        andIProvideRFMXAsX("contact name", "RFM test contact change")  // auto-chosen (score=1.00, RFMSteps.scala)
      ...
      When("I click change link for RFM New RFM CYA Change Contact preference")
        andIClickChangeLinkForRFMX("New RFM CYA Change Contact preference")  // auto-chosen (score=1.00, RFMSteps.scala)
        // --- Other possible matches ---
        // andIClickXLink() [0.83] (CommonSteps.scala) pattern: I click (.*) link
      ...
      When("I click change link for RFM Change Address")
        andIClickChangeLinkForRFMX("Change Address")  // auto-chosen (score=1.00, RFMSteps.scala)
        // --- Other possible matches ---
        // andIClickXLink() [0.83] (CommonSteps.scala) pattern: I click (.*) link

      And("I enter Address Line 1 as Address Line 1 CYA change")
        andIEnterXAsX("Address Line 1", "Address Line 1 CYA change")  // auto-chosen (score=1.00, UPESteps.scala)
    ...
    Scenario("3 - User registration as Limited liability partnership failed with party type mismatch error") {
      ...
      Then("I should navigate to RFM LLP GRS page")
        thenIShouldNavigateToX("RFM LLP GRS page")  // auto-chosen (score=1.00, EligibilityQuestionSteps.scala)
      ...
      Then("I should be on RFM GRS Registration Mismatch Error Page")
        thenIShouldBeOnX("RFM GRS Registration Mismatch Error Page")  // auto-chosen (score=1.00, EligibilityQuestionSteps.scala)
    ...
    Scenario("4 - User registration as UK limited company failed with identifiers mismatch error and entity type page validation") {
      ...
      Then("I should be on RFM GRS Registration Not Called Error Page")
        thenIShouldBeOnX("RFM GRS Registration Not Called Error Page")  // auto-chosen (score=1.00, EligibilityQuestionSteps.scala)
      ...
      Then("I should be on RFM New NFM Contact Name Page")
        thenIShouldBeOnX("RFM New NFM Contact Name Page")  // auto-chosen (score=1.00, EligibilityQuestionSteps.scala)
