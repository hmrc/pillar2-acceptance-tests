package uk.gov.hmrc.test.ui.specs
import org.scalatest.matchers.should.Matchers
import org.scalatest.featurespec.AnyFeatureSpec
import uk.gov.hmrc.test.ui.cucumber.stepdefs.Hooks.{And, Given, Then, When}
class RepaymentJourneysSpec extends AnyFeatureSpec with Matchers {

  Feature("Repayment Journey") {

    Scenario("1 - Validate error for Non UK bank account details page") {
      Given("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345674 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345674 for Pillar2 service

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      And("I access Non UK payment page")
        // ⚠️ No step-def match found for: I access Non UK payment page

      When("I click Sign out link")
        // ⚠️ No step-def match found for: I click Sign out link

      Then("I am on feedback survey page")
        // ⚠️ No step-def match found for: I am on feedback survey page

      When("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345674 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345674 for Pillar2 service

      And("I access Non UK payment page")
        // ⚠️ No step-def match found for: I access Non UK payment page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      And("I click Sign out link")
        // ⚠️ No step-def match found for: I click Sign out link

    }

    Scenario("2 - Organisation User navigates to repayment pages") {
      Given("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      When("I provide value for Refund Reason as Test Reason")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as Test Reason

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I select repayment method as UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as UK bank account

      Then("I should navigate to UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should navigate to UK Bank Account Payment Page

      And("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Method Page")
        // ⚠️ No step-def match found for: I should be on Repayment Method Page

      When("I select repayment method as Non-UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as Non-UK bank account

      Then("I should be on Non UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on Non UK Bank Account Payment Page

      When("I enter Non UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter Non UK Bank Account details as:

      And("I provide Repayment contact name as Repayment Contact Name")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name

      When("I provide Repayment contact email as repayment@email.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as repayment@email.com

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I provide Repayment contact phone as 789765423")
        // ⚠️ No step-def match found for: I provide Repayment contact phone as 789765423

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Phone Input Page")
        // ⚠️ No step-def match found for: I should be on Repayment Phone Input Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Phone Page")
        // ⚠️ No step-def match found for: I should be on Repayment Phone Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Contact Email Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Email Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I navigate from Contact page to CYA page")
        // ⚠️ No step-def match found for: I navigate from Contact page to CYA page

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Amount")
        // ⚠️ No step-def match found for: I click change link for Repayment Amount

      When("I provide Refund Amount as 1000.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 1000.00

      When("I click change link for Repayment Reason")
        // ⚠️ No step-def match found for: I click change link for Repayment Reason

      And("I provide Refund Reason as Test Reason")
        // ⚠️ No step-def match found for: I provide Refund Reason as Test Reason

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Bank Name")
        // ⚠️ No step-def match found for: I click change link for Repayment Bank Name

      When("I enter Non UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter Non UK Bank Account details as:

      When("I click change link for Repayment Contact Name")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Name

      And("I provide Repayment contact name as Repayment Contact Name change")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name change

      When("I click change link for Repayment Contact Email")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Email

      When("I provide Repayment contact email as email@change.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as email@change.com

      When("I click change link for Repayment Contact Phone")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Phone

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment UK Bank Method")
        // ⚠️ No step-def match found for: I click change link for Repayment UK Bank Method

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be redirect to Repayment processing page")
        // ⚠️ No step-def match found for: I should be redirect to Repayment processing page

      And("I should be on Repayment Confirmation Page")
        // ⚠️ No step-def match found for: I should be on Repayment Confirmation Page

      When("I click Back to group homepage link")
        // ⚠️ No step-def match found for: I click Back to group homepage link

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

    }

    Scenario("3 - Agent User navigates to repayment journey pages") {
      Given("Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service")
        // ⚠️ No step-def match found for: Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service

      And("I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service")
        // ⚠️ No step-def match found for: I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service

      Then("I should be on ASA Pillar2 Input Page")
        // ⚠️ No step-def match found for: I should be on ASA Pillar2 Input Page

      And("I provide ASA Pillar2 ID as XMPLR0012345674")
        // ⚠️ No step-def match found for: I provide ASA Pillar2 ID as XMPLR0012345674

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to ASA Confirmation Page")
        // ⚠️ No step-def match found for: I should navigate to ASA Confirmation Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 9999.99")
        // ⚠️ No step-def match found for: I provide Refund Amount as 9999.99

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      When("I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Method Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Method Page

      And("I select repayment method as UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as UK bank account

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on UK Bank Account Payment Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Method Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Method Page

      And("I should see the repayment method UK bank account remain selected")
        // ⚠️ No step-def match found for: I should see the repayment method UK bank account remain selected

      When("I select repayment method as Non-UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as Non-UK bank account

      When("I enter Non UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter Non UK Bank Account details as:

      And("I provide Repayment contact name as Repayment Contact Name")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name

      When("I provide Repayment contact email as repayment@email.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as repayment@email.com

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I provide Repayment contact phone as 789765423")
        // ⚠️ No step-def match found for: I provide Repayment contact phone as 789765423

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Phone Input Page")
        // ⚠️ No step-def match found for: I should be on Repayment Phone Input Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should be on Repayment Phone Input Page")
        // ⚠️ No step-def match found for: I should be on Repayment Phone Input Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Phone Page")
        // ⚠️ No step-def match found for: I should be on Repayment Phone Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Contact Email Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Email Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I navigate from Contact page to CYA page")
        // ⚠️ No step-def match found for: I navigate from Contact page to CYA page

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Amount")
        // ⚠️ No step-def match found for: I click change link for Repayment Amount

      When("I provide Refund Amount as 1000.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 1000.00

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Reason")
        // ⚠️ No step-def match found for: I click change link for Repayment Reason

      And("I provide Refund Reason as Test Reason")
        // ⚠️ No step-def match found for: I provide Refund Reason as Test Reason

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Bank Name")
        // ⚠️ No step-def match found for: I click change link for Repayment Bank Name

      And("I provide value for Bank Name as HSBC2")
        // ⚠️ No step-def match found for: I provide value for Bank Name as HSBC2

      And("I provide value for Account Name as Test Name2")
        // ⚠️ No step-def match found for: I provide value for Account Name as Test Name2

      And("I provide value for Swift Code as HBUKGB4C")
        // ⚠️ No step-def match found for: I provide value for Swift Code as HBUKGB4C

      And("I provide value for Iban as GB29NWBK60161331926820")
        // ⚠️ No step-def match found for: I provide value for Iban as GB29NWBK60161331926820

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Contact Name")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Name

      And("I provide Repayment contact name as Repayment Contact Name change")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name change

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Contact Email")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Email

      When("I provide Repayment contact email as email@change.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as email@change.com

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment Contact Phone")
        // ⚠️ No step-def match found for: I click change link for Repayment Contact Phone

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click change link for Repayment UK Bank Method")
        // ⚠️ No step-def match found for: I click change link for Repayment UK Bank Method

      When("I select repayment method as UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as UK bank account

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I provide value for UK Bank Name as Natwest")
        // ⚠️ No step-def match found for: I provide value for UK Bank Name as Natwest

      And("I provide value for UK Account Name as Epic Adventure Inc")
        // ⚠️ No step-def match found for: I provide value for UK Account Name as Epic Adventure Inc

      And("I provide value for Sort Code as 206705")
        // ⚠️ No step-def match found for: I provide value for Sort Code as 206705

      And("I provide value for UK Account number as 86473611")
        // ⚠️ No step-def match found for: I provide value for UK Account number as 86473611

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

    }

    Scenario("4 - Agent User navigates to repayment journey pages for UK bank account") {
      Given("Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service")
        // ⚠️ No step-def match found for: Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service

      And("I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service")
        // ⚠️ No step-def match found for: I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service

      Then("I should be on ASA Pillar2 Input Page")
        // ⚠️ No step-def match found for: I should be on ASA Pillar2 Input Page

      And("I provide ASA Pillar2 ID as XMPLR0012345674")
        // ⚠️ No step-def match found for: I provide ASA Pillar2 ID as XMPLR0012345674

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to ASA Confirmation Page")
        // ⚠️ No step-def match found for: I should navigate to ASA Confirmation Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 999.99")
        // ⚠️ No step-def match found for: I provide Refund Amount as 999.99

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      When("I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Method Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Method Page

      And("I select repayment method as UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as UK bank account

      Then("I should be on UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on UK Bank Account Payment Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should navigate to Repayment Method Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Method Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on UK Bank Account Payment Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I refresh the page")
        // ⚠️ No step-def match found for: I refresh the page

      When("I enter UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter UK Bank Account details as:

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I provide Repayment contact name as Repayment Contact Name")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name

      When("I provide Repayment contact email as repayment@email.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as repayment@email.com

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I provide Repayment contact phone as 789765423")
        // ⚠️ No step-def match found for: I provide Repayment contact phone as 789765423

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      And("I select back link")
        // ⚠️ No step-def match found for: I select back link

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      And("I click Sign out link")
        // ⚠️ No step-def match found for: I click Sign out link

    }

    Scenario("5 - Organisation User navigates to repayment journey pages for UK bank account") {
      Given("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

      When("I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as A content designer works on the end-to-end journey of a service to help users complete their goal and government deliver a policy intent. Their work may involve the creation of, or change to, a transaction, product or single piece of content.

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Method Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Method Page

      And("I select repayment method as UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as UK bank account

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should navigate to UK Bank Account Payment Page

      When("I enter UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter UK Bank Account details as:

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should navigate to UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should navigate to UK Bank Account Payment Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I provide Repayment contact name as Repayment Contact Name")
        // ⚠️ No step-def match found for: I provide Repayment contact name as Repayment Contact Name

      When("I provide Repayment contact email as repayment@email.com")
        // ⚠️ No step-def match found for: I provide Repayment contact email as repayment@email.com

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I provide Repayment contact phone as 789765423")
        // ⚠️ No step-def match found for: I provide Repayment contact phone as 789765423

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

    }

    Scenario("6 - Organisation User navigates to error page when repayment submission API fails to submit data to ETMP") {
      Given("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      And("I provide value for Refund Reason as Test Refund")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as Test Refund

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I access Repayment CYA page")
        // ⚠️ No step-def match found for: I access Repayment CYA page

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click Continue button")
        // ⚠️ No step-def match found for: I click Continue button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click Continue button")
        // ⚠️ No step-def match found for: I click Continue button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I click go back and complete all the required answers link")
        // ⚠️ No step-def match found for: I click go back and complete all the required answers link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Reason For Refund Page")
        // ⚠️ No step-def match found for: I should navigate to Reason For Refund Page

    }

    Scenario("7 - Agent User navigates to error page when repayment submission API fails to submit data to ETMP") {
      Given("Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service")
        // ⚠️ No step-def match found for: Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service

      And("I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service")
        // ⚠️ No step-def match found for: I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service

      Then("I should be on ASA Pillar2 Input Page")
        // ⚠️ No step-def match found for: I should be on ASA Pillar2 Input Page

      And("I provide ASA Pillar2 ID as XMPLR0012345674")
        // ⚠️ No step-def match found for: I provide ASA Pillar2 ID as XMPLR0012345674

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to ASA Confirmation Page")
        // ⚠️ No step-def match found for: I should navigate to ASA Confirmation Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click Request a repayment link")
        // ⚠️ No step-def match found for: I click Request a repayment link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Repayment Amount Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Amount Page

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      And("I provide value for Refund Reason as Test Refund")
        // ⚠️ No step-def match found for: I provide value for Refund Reason as Test Refund

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      And("I access Repayment CYA page")
        // ⚠️ No step-def match found for: I access Repayment CYA page

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click Continue button")
        // ⚠️ No step-def match found for: I click Continue button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I select back link")
        // ⚠️ No step-def match found for: I select back link

      Then("I should be on Repayment CYA Page")
        // ⚠️ No step-def match found for: I should be on Repayment CYA Page

      When("I click Continue button")
        // ⚠️ No step-def match found for: I click Continue button

      Then("I should be on Repayment Incomplete Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Incomplete Error Page

      When("I click go back and complete all the required answers link")
        // ⚠️ No step-def match found for: I click go back and complete all the required answers link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

    }

    Scenario("8 - Organisation User navigates to journey recovery error page when they try to skip mandatory questions") {
      Given("Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service")
        // ⚠️ No step-def match found for: Organisation User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345676 for Pillar2 service

      Then("I should be on Dashboard page")
        // ⚠️ No step-def match found for: I should be on Dashboard page

      When("I click Request a refund link")
        // ⚠️ No step-def match found for: I click Request a refund link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      And("I provide Refund Reason as Test Reason")
        // ⚠️ No step-def match found for: I provide Refund Reason as Test Reason

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment Method Page")
        // ⚠️ No step-def match found for: I should be on Repayment Method Page

      When("I select repayment method as Non-UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as Non-UK bank account

      Then("I should be on Non UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on Non UK Bank Account Payment Page

      When("I enter Non UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter Non UK Bank Account details as:

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I access Repayment contact email page")
        // ⚠️ No step-def match found for: I access Repayment contact email page

      Then("I should be on Repayment Journey Recovery Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Journey Recovery Error Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should be on Repayment Journey Recovery Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Journey Recovery Error Page

      When("I click Return to account homepage link")
        // ⚠️ No step-def match found for: I click Return to account homepage link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

    }

    Scenario("9 - Agent User navigates to journey recovery error page when they try to skip mandatory questions") {
      Given("Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service")
        // ⚠️ No step-def match found for: Agent User logs in with existing entity group HMRC-AS-AGENT, AgentReference and 1234 for Pillar2 service

      And("I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service")
        // ⚠️ No step-def match found for: I add delegated enrolment with HMRC-PILLAR2-ORG, PLRID, XMPLR0012345674 and pillar2-auth for Pillar2 service

      Then("I should be on ASA Pillar2 Input Page")
        // ⚠️ No step-def match found for: I should be on ASA Pillar2 Input Page

      And("I provide ASA Pillar2 ID as XMPLR0012345674")
        // ⚠️ No step-def match found for: I provide ASA Pillar2 ID as XMPLR0012345674

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to ASA Confirmation Page")
        // ⚠️ No step-def match found for: I should navigate to ASA Confirmation Page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click Request a refund link")
        // ⚠️ No step-def match found for: I click Request a refund link

      Then("I should navigate to Repayment Guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Repayment Guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I provide Refund Amount as 100.00")
        // ⚠️ No step-def match found for: I provide Refund Amount as 100.00

      And("I provide Refund Reason as Test Reason")
        // ⚠️ No step-def match found for: I provide Refund Reason as Test Reason

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should be on Repayment Method Page")
        // ⚠️ No step-def match found for: I should be on Repayment Method Page

      When("I select repayment method as Non-UK bank account")
        // ⚠️ No step-def match found for: I select repayment method as Non-UK bank account

      Then("I should be on Non UK Bank Account Payment Page")
        // ⚠️ No step-def match found for: I should be on Non UK Bank Account Payment Page

      When("I enter Non UK Bank Account details as:")
        // ⚠️ No step-def match found for: I enter Non UK Bank Account details as:

      Then("I should be on Repayment Contact Page")
        // ⚠️ No step-def match found for: I should be on Repayment Contact Page

      When("I access Repayment contact email page")
        // ⚠️ No step-def match found for: I access Repayment contact email page

      Then("I should be on Repayment Journey Recovery Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Journey Recovery Error Page

      When("I click Report Pillar 2 Top-up Taxes link")
        // ⚠️ No step-def match found for: I click Report Pillar 2 Top-up Taxes link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

      When("I click the browser back button")
        // ⚠️ No step-def match found for: I click the browser back button

      Then("I should be on Repayment Journey Recovery Error Page")
        // ⚠️ No step-def match found for: I should be on Repayment Journey Recovery Error Page

      When("I click Return to account homepage link")
        // ⚠️ No step-def match found for: I click Return to account homepage link

      Then("I should navigate to Dashboard page")
        // ⚠️ No step-def match found for: I should navigate to Dashboard page

    }
  }
}
