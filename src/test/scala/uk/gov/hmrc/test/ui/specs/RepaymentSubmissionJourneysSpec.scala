...
class RepaymentSubmissionJourneysSpec extends BaseSpec with Matchers {
  ...
  Feature("Repayment Journey") {
    Scenario("1 - Org User successfully submits repayment journey with a UK bank account", Tests) {
      ...
      RepaymentConfirmationPage.onPage(timeoutSeconds = 10)
    }

    Scenario("2 - Org User successfully submits repayment journey with a Non-UK bank account", Tests) {
      Given("Org User logs in with existing entity group HMRC-PILLAR2-ORG, PLRID and XMPLR0012345674 for Pillar2 service")
      login(
        userType = "Organisation",
        pageUrl = "dashboard",
    ...
    Scenario("3 - Agent User successfully submits repayment journey with a UK bank account", Tests) {
