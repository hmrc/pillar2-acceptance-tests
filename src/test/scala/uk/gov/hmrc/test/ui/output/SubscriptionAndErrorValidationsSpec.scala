package uk.gov.hmrc.test.ui.specs
import org.scalatest.matchers.should.Matchers
import org.scalatest.featurespec.AnyFeatureSpec
import uk.gov.hmrc.test.ui.cucumber.stepdefs.Hooks.{And, Given, Then, When}
class SubscriptionAndErrorValidationsSpec extends AnyFeatureSpec with Matchers {

  Feature("Create a new registration and subscription") {

    Scenario("1 - User redirected to a placeholder error page, for a duplicate subscription on UPE No ID flow,UPE as DFM path.") {
      Given("Organisation User logs in to subscribe for Pillar2")
        // ⚠️ No step-def match found for: Organisation User logs in to subscribe for Pillar2

      When("I click Add Ultimate Parent Entity details link")
        // ⚠️ No step-def match found for: I click Add Ultimate Parent Entity details link

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      And("I enter UPE name as XMPLR0009999999")
        // ⚠️ No step-def match found for: I enter UPE name as XMPLR0009999999

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      When("I enter UPE Person/Team name as UPE Contact Name")
        // ⚠️ No step-def match found for: I enter UPE Person/Team name as UPE Contact Name

      When("I enter UPE Email address as testteam@email.com")
        // ⚠️ No step-def match found for: I enter UPE Email address as testteam@email.com

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should be on Check your answers page")
        // ⚠️ No step-def match found for: I should be on Check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add filing member details link")
        // ⚠️ No step-def match found for: I click Add filing member details link

      When("I select No option and continue to next")
        // ⚠️ No step-def match found for: I select No option and continue to next

      When("I click Add further group details link")
        // ⚠️ No step-def match found for: I click Add further group details link

      When("I select option In the UK and outside the UK in further details group status page")
        // ⚠️ No step-def match found for: I select option In the UK and outside the UK in further details group status page

      When("I enter account period as:")
        // ⚠️ No step-def match found for: I enter account period as:

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add contact details link")
        // ⚠️ No step-def match found for: I click Add contact details link

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      Then("I should navigate to Contact details Check answers page")
        // ⚠️ No step-def match found for: I should navigate to Contact details Check answers page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Check your answers before submitting your registration link")
        // ⚠️ No step-def match found for: I click Check your answers before submitting your registration link

      Then("I should navigate to Review answers page")
        // ⚠️ No step-def match found for: I should navigate to Review answers page

      And("I click on Save&Continue button")
        // ⚠️ No step-def match found for: I click on Save&Continue button

      Then("I should navigate to duplicate ack ref error page")
        // ⚠️ No step-def match found for: I should navigate to duplicate ack ref error page

    }

    Scenario("2 - User redirected to a placeholder error page, for an incomplete data submission during registration") {
      Given("Organisation User logs in without Pillar2 enrolment")
        // ⚠️ No step-def match found for: Organisation User logs in without Pillar2 enrolment

      Then("I should be on UPE business page")
        // ⚠️ No step-def match found for: I should be on UPE business page

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      When("I select option UK limited company and continue to GRS page")
        // ⚠️ No step-def match found for: I select option UK limited company and continue to GRS page

      Then("I should navigate to UKCompany GRS page")
        // ⚠️ No step-def match found for: I should navigate to UKCompany GRS page

      When("I registered successfully with BV enabled")
        // ⚠️ No step-def match found for: I registered successfully with BV enabled

      And("I click on Save&Continue button")
        // ⚠️ No step-def match found for: I click on Save&Continue button

      When("I click Add filing member details link")
        // ⚠️ No step-def match found for: I click Add filing member details link

      When("I select Yes option and continue to next")
        // ⚠️ No step-def match found for: I select Yes option and continue to next

      Then("I should navigate to NFM details page")
        // ⚠️ No step-def match found for: I should navigate to NFM details page

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      Then("I should be on NFM entity type page")
        // ⚠️ No step-def match found for: I should be on NFM entity type page

      When("I select option UK limited company and continue to GRS page")
        // ⚠️ No step-def match found for: I select option UK limited company and continue to GRS page

      Then("I should navigate to NFM UKCompany GRS page")
        // ⚠️ No step-def match found for: I should navigate to NFM UKCompany GRS page

      And("I click on Save&Continue button")
        // ⚠️ No step-def match found for: I click on Save&Continue button

      When("I click Add further group details link")
        // ⚠️ No step-def match found for: I click Add further group details link

      When("I select option In the UK and outside the UK in further details group status page")
        // ⚠️ No step-def match found for: I select option In the UK and outside the UK in further details group status page

      When("I enter account period as:")
        // ⚠️ No step-def match found for: I enter account period as:

      Then("I should navigate to FD check your answers page")
        // ⚠️ No step-def match found for: I should navigate to FD check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Task list page")
        // ⚠️ No step-def match found for: I should navigate to Task list page

      When("I click Add contact details link")
        // ⚠️ No step-def match found for: I click Add contact details link

      Then("I should navigate to Contact details guidance page")
        // ⚠️ No step-def match found for: I should navigate to Contact details guidance page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I enter Contact Name as Contact Name Test")
        // ⚠️ No step-def match found for: I enter Contact Name as Contact Name Test

      When("I enter Contact Email as testContact@email.com")
        // ⚠️ No step-def match found for: I enter Contact Email as testContact@email.com

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should navigate to Second Contact details page")
        // ⚠️ No step-def match found for: I should navigate to Second Contact details page

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      Then("I should navigate to Contact details Check answers page")
        // ⚠️ No step-def match found for: I should navigate to Contact details Check answers page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Check your answers before submitting your registration link")
        // ⚠️ No step-def match found for: I click Check your answers before submitting your registration link

      When("I click on change link for Primary Contact")
        // ⚠️ No step-def match found for: I click on change link for Primary Contact

      Then("I should navigate to Contact details change phone page")
        // ⚠️ No step-def match found for: I should navigate to Contact details change phone page

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      And("I select back link")
        // ⚠️ No step-def match found for: I select back link

      And("I select back link")
        // ⚠️ No step-def match found for: I select back link

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Incomplete Data Submission Error Page")
        // ⚠️ No step-def match found for: I should navigate to Incomplete Data Submission Error Page

      When("I click go back to register your group and complete any in progress tasks. link")
        // ⚠️ No step-def match found for: I click go back to register your group and complete any in progress tasks. link

      Then("I should navigate to Task list page")
        // ⚠️ No step-def match found for: I should navigate to Task list page

      Then("The Task Add contact details status should be In progress")
        // ⚠️ No step-def match found for: The Task Add contact details status should be In progress

    }

    Scenario("3 - User redirected to a placeholder error page, for an invalid request during registration.") {
      Given("Organisation User logs in to subscribe for Pillar2")
        // ⚠️ No step-def match found for: Organisation User logs in to subscribe for Pillar2

      Then("I should be on Task list page")
        // ⚠️ No step-def match found for: I should be on Task list page

      When("I click Add Ultimate Parent Entity details link")
        // ⚠️ No step-def match found for: I click Add Ultimate Parent Entity details link

      Then("I should navigate to Initial guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Initial guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to UPE business page")
        // ⚠️ No step-def match found for: I should navigate to UPE business page

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should navigate to input-upe-name page")
        // ⚠️ No step-def match found for: I should navigate to input-upe-name page

      And("I enter UPE name as regNoIDInvalidRequest")
        // ⚠️ No step-def match found for: I enter UPE name as regNoIDInvalidRequest

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      When("I enter UPE Person/Team name as UPE Contact Name")
        // ⚠️ No step-def match found for: I enter UPE Person/Team name as UPE Contact Name

      When("I enter UPE Email address as testteam@email.com")
        // ⚠️ No step-def match found for: I enter UPE Email address as testteam@email.com

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should be on Check your answers page")
        // ⚠️ No step-def match found for: I should be on Check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add filing member details link")
        // ⚠️ No step-def match found for: I click Add filing member details link

      Then("I should navigate to NFM registration page")
        // ⚠️ No step-def match found for: I should navigate to NFM registration page

      When("I select No option and continue to next")
        // ⚠️ No step-def match found for: I select No option and continue to next

      When("I click Add further group details link")
        // ⚠️ No step-def match found for: I click Add further group details link

      When("I select option In the UK and outside the UK in further details group status page")
        // ⚠️ No step-def match found for: I select option In the UK and outside the UK in further details group status page

      When("I enter account period as:")
        // ⚠️ No step-def match found for: I enter account period as:

      Then("I should navigate to FD check your answers page")
        // ⚠️ No step-def match found for: I should navigate to FD check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add contact details link")
        // ⚠️ No step-def match found for: I click Add contact details link

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Contact details display page")
        // ⚠️ No step-def match found for: I should navigate to Contact details display page

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      Then("I should navigate to Second Contact details page")
        // ⚠️ No step-def match found for: I should navigate to Second Contact details page

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      Then("I should navigate to Contact details Check answers page")
        // ⚠️ No step-def match found for: I should navigate to Contact details Check answers page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Check your answers before submitting your registration link")
        // ⚠️ No step-def match found for: I click Check your answers before submitting your registration link

      And("I click on Save&Continue button")
        // ⚠️ No step-def match found for: I click on Save&Continue button

      Then("I should navigate to Registration API error page")
        // ⚠️ No step-def match found for: I should navigate to Registration API error page

      When("I click Return to registration to try again link")
        // ⚠️ No step-def match found for: I click Return to registration to try again link

      Then("I should navigate to Task list page")
        // ⚠️ No step-def match found for: I should navigate to Task list page

    }

    Scenario("4 - Verify the link on page can't be found page") {
      Given("Organisation User logs in without Pillar2 enrolment")
        // ⚠️ No step-def match found for: Organisation User logs in without Pillar2 enrolment

      And("I access random page")
        // ⚠️ No step-def match found for: I access random page

      Then("I click return to your Pillar 2 Top-up Taxes registration link")
        // ⚠️ No step-def match found for: I click return to your Pillar 2 Top-up Taxes registration link

      Then("I should see task list sections")
        // ⚠️ No step-def match found for: I should see task list sections

      And("The Task Add Ultimate Parent Entity details status should be Not started")
        // ⚠️ No step-def match found for: The Task Add Ultimate Parent Entity details status should be Not started

    }

    Scenario("5 - User redirected to a placeholder error page, for a duplicate subscription on UPE No ID flow,UPE as DFM path.") {
      Given("Organisation User logs in to subscribe for Pillar2")
        // ⚠️ No step-def match found for: Organisation User logs in to subscribe for Pillar2

      Then("I should be on Task list page")
        // ⚠️ No step-def match found for: I should be on Task list page

      When("I click Add Ultimate Parent Entity details link")
        // ⚠️ No step-def match found for: I click Add Ultimate Parent Entity details link

      Then("I should navigate to Initial guidance Page")
        // ⚠️ No step-def match found for: I should navigate to Initial guidance Page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to UPE business page")
        // ⚠️ No step-def match found for: I should navigate to UPE business page

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      And("I enter UPE name as Test UPE")
        // ⚠️ No step-def match found for: I enter UPE name as Test UPE

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      Then("I should navigate to UPE Contact person/team Name page")
        // ⚠️ No step-def match found for: I should navigate to UPE Contact person/team Name page

      When("I enter UPE Person/Team name as UPE Contact Name")
        // ⚠️ No step-def match found for: I enter UPE Person/Team name as UPE Contact Name

      When("I enter UPE Email address as testteam@email.com")
        // ⚠️ No step-def match found for: I enter UPE Email address as testteam@email.com

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      Then("I should be on Check your answers page")
        // ⚠️ No step-def match found for: I should be on Check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add filing member details link")
        // ⚠️ No step-def match found for: I click Add filing member details link

      Then("I should navigate to NFM registration page")
        // ⚠️ No step-def match found for: I should navigate to NFM registration page

      When("I select No option and continue to next")
        // ⚠️ No step-def match found for: I select No option and continue to next

      When("I click Add further group details link")
        // ⚠️ No step-def match found for: I click Add further group details link

      When("I select option In the UK and outside the UK in further details group status page")
        // ⚠️ No step-def match found for: I select option In the UK and outside the UK in further details group status page

      When("I enter account period as:")
        // ⚠️ No step-def match found for: I enter account period as:

      Then("I should navigate to FD check your answers page")
        // ⚠️ No step-def match found for: I should navigate to FD check your answers page

      And("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Add contact details link")
        // ⚠️ No step-def match found for: I click Add contact details link

      Then("I should navigate to Contact details guidance page")
        // ⚠️ No step-def match found for: I should navigate to Contact details guidance page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      Then("I should navigate to Contact details display page")
        // ⚠️ No step-def match found for: I should navigate to Contact details display page

      When("I select option Yes and continue to next")
        // ⚠️ No step-def match found for: I select option Yes and continue to next

      Then("I should navigate to Second Contact details page")
        // ⚠️ No step-def match found for: I should navigate to Second Contact details page

      When("I select option No and continue to next")
        // ⚠️ No step-def match found for: I select option No and continue to next

      When("I enter Address as:")
        // ⚠️ No step-def match found for: I enter Address as:

      Then("I should navigate to Contact details Check answers page")
        // ⚠️ No step-def match found for: I should navigate to Contact details Check answers page

      When("I click on Continue button")
        // ⚠️ No step-def match found for: I click on Continue button

      When("I click Check your answers before submitting your registration link")
        // ⚠️ No step-def match found for: I click Check your answers before submitting your registration link

      Then("I should navigate to Review answers page")
        // ⚠️ No step-def match found for: I should navigate to Review answers page

      And("I should see Print this page link on Review answers page")
        // ⚠️ No step-def match found for: I should see Print this page link on Review answers page

    }
  }
}
