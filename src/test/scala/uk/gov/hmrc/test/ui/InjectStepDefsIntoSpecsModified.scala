...
object InjectStepDefsIntoSpecsModified {
  ...
  def loadStepDefs(stepDefsDir: File): Seq[StepDef] = {
    ...
    //    val files = Option(stepDefsDir.listFiles()).map(Seq.unsafeWrapArray).getOrElse(Seq.empty[File])
    val files = listScalaFilesRecursive(stepDefsDir)
    files.filter(_.getName.endsWith(".scala")).flatMap { file =>
      val buffered = Source.fromFile(file)
      val lines =
        try {
          buffered.getLines().toList
        } finally {
          buffered.close()
        }
  ...
  def processSpecFile(specFile: File, stepDefsDir: File, outputDir: File): Unit = {
    ...
    val buffered = Source.fromFile(specFile)
    val lines =
      try {
        buffered.getLines().toList
      } finally {
  ...
  def processFolder(specDir: String, stepDefsDir: String, outputDir: String): Unit = {
    ...
    val list = Option(in.listFiles()).map(_.toSeq).getOrElse(Seq.empty)
