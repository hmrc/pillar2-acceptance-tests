...
class StepDef extends BaseStepDef with BrowserDriver {
  ...
  Given("""^(.*) logs in to register for Pillar2 Agent service$""") {
    name: String =>
      name match {
  ...
  Given("""^(.*) logs in as upe with credId (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
        case "Organisation User" =>
          AuthLoginPage.loginToUPEWithCredID(name, credId)
        case _ => AuthLoginPage.loginToUPEWithCredID(name, credId)
      }
  }

  Given("""^(.*) logs in with credId (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
        case "Organisation User" =>
          AuthLoginPage.loginAsUserWithCredId(name, credId)
        case _ => AuthLoginPage.loginAsUserWithCredId(name, credId)
      }
  }
  Given("""^(.*) logs in to upe org page with CredID (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
        case "Organisation User" =>
          AuthLoginPage.loginToOrgWithCredID(name, credId)
        case _ => AuthLoginPage.loginToOrgWithCredID(name, credId)
      }
  }

  Given("""^(.*) logs in to nfm org page with CredID (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
        case "Organisation User" =>
          AuthLoginPage.loginToNfmOrgWithCredID(name, credId)
  ...
  Given(
    ...
    name match {
      case "Organisation User" =>
        AuthLoginPage.loginToRegWithCredID(name, credId)
      case _ => AuthLoginPage.loginToRegWithCredID(name, credId)
    }
  }
  Given("""^(.*) logs in to upe name page with CredID (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
  ...
  Given("""^(.*) logs in to nfm name page with CredID (.*) for Pillar2$""") {
    (name: String, credId: String) =>
      name match {
        case "Organisation User" =>
          AuthLoginPage.loginToNFMNameWithCredID(name, credId)
  ...
  Given(
    ...
    name match {
      ...
      case "Final Check Your Answers" =>
        AuthLoginPage.loginToFinalCA(name, credId)
    }
  }

  When("""^Organisation User logs in to (.*) page with credId (.*)$""") {
    (name: String, credId: String) =>
      name match {
  ...
  Then("""^I clear the cache$""") {
    Nav.navigateTo(
      "http://localhost:10050/report-pillar2-top-up-taxes/test-only/eligibility/clear-session"
    )
  }
  ...
  Then("""^The hint text should be (.*)$""") { header: String =>
    ...
    assert(
      getTextOf(By.cssSelector(BTAPillar2IDCheckPage.hintText)).contains(header)
    )
  }
  ...
  Given("""^I fill (.*) and continue$""") { page: String =>
    page match {
      case "What is the main address of your business page" =>
        Forms.addressNonUK()
    }
    Input.clickSubmit()
  }
  ...
  Given("""^I am on (.*) Page$""") { page: String =>
    page match {
      ...
      case "NFM registration failed error" =>
        ...
        isVisible(
          By.cssSelector(NFMGRSRegistrationFailedErrorPage.header)
        ) shouldBe true
      case "NFM registration not called error" =>
        navigateTo(NFMGRSRegistrationNotCalledErrorPage.url)
        Wait.waitForElementToPresentByCssSelector(
          NFMGRSRegistrationNotCalledErrorPage.content
        )
        isVisible(
      ...
      case "UPE registration failed error" =>
        ...
        isVisible(
          By.cssSelector(UPEGRSRegistrationFailedErrorPage.header)
        ) shouldBe true
      case "UPE registration not called error" =>
        navigateTo(UPEGRSRegistrationNotCalledErrorPage.url)
        Wait.waitForElementToPresentByCssSelector(
          UPEGRSRegistrationNotCalledErrorPage.content
        )
        isVisible(
          By.cssSelector(UPEGRSRegistrationNotCalledErrorPage.header)
        ) shouldBe true
    }
  }

  And("""^I should see error message (.*) on the (.*) Page$""") {
    (error: String, page: String) =>
      page match {
        case "Eligibility question" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            BusinessActivityEQPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            BusinessActivityEQPage.errorLink
          )
          getTextOf(
            By cssSelector (BusinessActivityEQPage.errorLink)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            BusinessActivityEQPage.errorMessage
          )
          getTextOf(
            By cssSelector (BusinessActivityEQPage.errorMessage)
          ) should include(error)

        case "Global gross revenue" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            GlobalGrossRevenueEQPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            GlobalGrossRevenueEQPage.errorLink
          )
          getTextOf(
            By cssSelector (GlobalGrossRevenueEQPage.errorLink)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            GlobalGrossRevenueEQPage.errorMessage
          )
          getTextOf(
            By cssSelector (GlobalGrossRevenueEQPage.errorMessage)
          ) should include(error)
        ...
        case "UPE Org type" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            UPEEntityTypePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(UPEEntityTypePage.errorLink)
          getTextOf(By cssSelector (UPEEntityTypePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            UPEEntityTypePage.errorMessage
          )
          getTextOf(
            By cssSelector (UPEEntityTypePage.errorMessage)
          ) should include(error)

        case "NFM Org type" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            NFMEntityTypePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(NFMEntityTypePage.errorLink)
          getTextOf(By cssSelector (NFMEntityTypePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            NFMEntityTypePage.errorMessage
          )
          getTextOf(
            By cssSelector (NFMEntityTypePage.errorMessage)
          ) should include(error)
        ...
        case "UPE Telephone" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            UPETelephonePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(UPETelephonePage.errorLink)
          getTextOf(By cssSelector (UPETelephonePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            UPETelephonePage.errorMessage
          )
          getTextOf(
            By cssSelector (UPETelephonePage.errorMessage)
          ) should include(error)

        case "Input Telephone" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            InputUPETelephonePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            InputUPETelephonePage.errorLink
          )
          getTextOf(By cssSelector (InputUPETelephonePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            InputUPETelephonePage.errorMessage
          )
          getTextOf(
            By cssSelector (InputUPETelephonePage.errorMessage)
          ) should include(error)

        case "UPE Contact person/team name" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            UPEContactNamePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            UPEContactNamePage.errorLink
          )
          getTextOf(By cssSelector (UPEContactNamePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            UPEContactNamePage.errorMessage
          )
          getTextOf(
            By cssSelector (UPEContactNamePage.errorMessage)
          ) should include(error)

        case "UPE contact email" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            UPEContactEmailPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            UPEContactEmailPage.errorLink
          )
          getTextOf(By cssSelector (UPEContactEmailPage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            UPEContactEmailPage.errorMessage
          )
          getTextOf(
            By cssSelector (UPEContactEmailPage.errorMessage)
          ) should include(error)
        ...
        case "NFM Telephone" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            NFMTelephonePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(NFMTelephonePage.errorLink)
          getTextOf(By cssSelector (NFMTelephonePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            NFMTelephonePage.errorMessage
          )
          getTextOf(
            By cssSelector (NFMTelephonePage.errorMessage)
          ) should include(error)

        case "Input Nfm Telephone" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            InputNFMTelephonePage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            InputNFMTelephonePage.errorLink
          )
          getTextOf(By cssSelector (InputNFMTelephonePage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            InputNFMTelephonePage.errorMessage
          )
          getTextOf(
            By cssSelector (InputNFMTelephonePage.errorMessage)
          ) should include(error)

        case "Further Details Group Status" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            FDGroupStatusPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(FDGroupStatusPage.errorLink)
          getTextOf(By cssSelector (FDGroupStatusPage.errorLink)) should be(
            error
          )

          Wait.waitForElementToPresentByCssSelector(
            FDGroupStatusPage.errorMessage
          )
          getTextOf(
            By cssSelector (FDGroupStatusPage.errorMessage)
          ) should include(error)

        case "Group Accounting Period Start Date" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            GroupAccountingPeriodPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            GroupAccountingPeriodPage.errorLinkStartDate
          )
          getTextOf(
            By cssSelector (GroupAccountingPeriodPage.errorLinkStartDate)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            GroupAccountingPeriodPage.errorMessageStartDate
          )
          getTextOf(
            By cssSelector (GroupAccountingPeriodPage.errorMessageStartDate)
          ) should include(error)
        ...
        case "Group Accounting Period End Date" =>
          ...
          Wait.waitForElementToPresentByCssSelector(
            GroupAccountingPeriodPage.errorLinkEndDate
          )
          getTextOf(
            By cssSelector (GroupAccountingPeriodPage.errorLinkEndDate)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            GroupAccountingPeriodPage.errorMessageEndDate
          )
          getTextOf(
            By cssSelector (GroupAccountingPeriodPage.errorMessageEndDate)
          ) should include(error)

        case "Contact details display" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            ContactDetailsDisplayPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            ContactDetailsDisplayPage.errorLink
          )
          getTextOf(
            By cssSelector (ContactDetailsDisplayPage.errorLink)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            ContactDetailsDisplayPage.errorMessage
          )
          getTextOf(
            By cssSelector (ContactDetailsDisplayPage.errorMessage)
          ) should include(error)

        case "Second Contact details" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            SecondContactDetailsDisplayPage.errorSummary
          )

          Wait.waitForElementToPresentByCssSelector(
            SecondContactDetailsDisplayPage.errorLink
          )
          getTextOf(
            By cssSelector (SecondContactDetailsDisplayPage.errorLink)
          ) should be(error)

          Wait.waitForElementToPresentByCssSelector(
            SecondContactDetailsDisplayPage.errorMessage
          )
          getTextOf(
            By cssSelector (SecondContactDetailsDisplayPage.errorMessage)
          ) should include(error)
        ...
        case "BTA Pillar2 validation" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            BTAPillar2IDCheckPage.errorMessage
          )

          Wait.waitForElementToPresentByCssSelector(
            BTAPillar2IDCheckPage.errorLink
          )
          getTextOf(By cssSelector (BTAPillar2IDCheckPage.errorLink)) should be(
            error
          )
          ...
          getTextOf(
            By cssSelector (BTAPillar2IDCheckPage.errorMessage)
          ) should include(error)

        case "Reason For Refund" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            RepaymentReasonPage.errorMessage
          )

          Wait.waitForElementToPresentByCssSelector(
            RepaymentReasonPage.errorLink
          )
          getTextOf(By cssSelector (RepaymentReasonPage.errorLink)) should be(
            error
          )
          ...
          getTextOf(
            By cssSelector (RepaymentReasonPage.errorMessage)
          ) should include(error)

        case "Repayment Method" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            RepaymentMethodPage.errorMessage
          )

          Wait.waitForElementToPresentByCssSelector(
            RepaymentMethodPage.errorLink
          )
          getTextOf(By cssSelector (RepaymentMethodPage.errorLink)) should be(
            error
          )
          ...
          getTextOf(
            By cssSelector (RepaymentMethodPage.errorMessage)
          ) should include(error)

        case "Agent Repayment Method" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            AgentRepaymentMethodPage.errorMessage
          )

          Wait.waitForElementToPresentByCssSelector(
            AgentRepaymentMethodPage.errorLink
          )
          getTextOf(
            By cssSelector (AgentRepaymentMethodPage.errorLink)
          ) should be(error)
          ...
          getTextOf(
            By cssSelector (AgentRepaymentMethodPage.errorMessage)
          ) should include(error)
      }
  }
  ...
  Then("""^The caption must be (.*)$""") { caption: String =>
    ...
    assert(
      getTextOf(By.cssSelector(InitialGuidancePage.caption)).equals(caption)
    )
  }
  ...
  And(
    """^I should see the contact details row (\d+) as (.*) on use contact page"""
  ) { (row: Int, details: String) =>
    Wait.waitForTagNameToBeRefreshed("h1")
    assert(
      driver
        .findElements(By.cssSelector(ContactDetailsDisplayPage.valueList))
        .get(row - 1)
        .getText
        .contains(details)
    )
  ...
  Then("""^I should be navigated to (.*) page$""") { (text: String) =>
    ...
    assert(
      driver
        .findElement(By.cssSelector(UPEPage.sendyourfeedback))
        .getText
        .contains(text)
    )
  }
  ...
  And("""^I should see (.*) hyperLink$""") { (linkText: String) =>
    Wait.waitForTagNameToBeRefreshed("h1")
    Wait.waitForElementToPresentByCssSelector(
      UPEEntityTypePage.inputUpeNamePageLink
    )
  }
  ...
  And("""^I should see (.*) link$""") { (linkText: String) =>
    Wait.waitForTagNameToBeRefreshed("h1")
    Wait.waitForElementToPresentByCssSelector(
      BTARegisterGuidancePage.registerLink
    )
    assert(
      driver
        .findElement(By.cssSelector(BTARegisterGuidancePage.registerLink))
        .getText
        .contains(linkText)
    )
  }
  ...
  And("""^I should see (.*) BTA button$""") { (option: String) =>
    option match {
      case "return to" =>
        Wait.waitForTagNameToBeRefreshed("h1")
        Wait.waitForElementToPresentByCssSelector(
          BTARegisterGuidancePage.returnToBTA
        )
    }
  }
  ...
  When(
    ...
    (
      ...
      userType match {
        case "Organisation" =>
          AuthLoginPage.loginWithExistingEntity(
            enrolmentkey,
            identifiername,
            identifiervalue
          )
  ...
  When(
    ...
    (
      ...
      AuthLoginPage.addDelegatedEnrolment(
        enrolmentkey,
        identifiername,
        identifiervalue,
        authRule
    ...
    linkText match {
      case "Print this page" =>
        ...
        assert(
          driver
            .findElement(
              By.cssSelector(RegistrationConfirmationPage.printthispage)
            )
            .getText
            .contains(linkText)
        )
      case "Agent Services Account" =>
        ...
        assert(
          driver
            .findElement(By.cssSelector(ASADashboardPage.ASALink))
            .getText
            .contains(linkText)
        )
      case "Sign out" =>
        ...
        assert(
          driver
            .findElement(By.cssSelector(RepaymentConfirmationPage.signOut))
            .getText
            .contains(linkText)
        )
    }
  ...
  And("""^I should see (.*) link on (.*)$""") {
    (linkText: String, page: String) =>
      page match {
        case "Review answers page" =>
          Wait.waitForTagNameToBeRefreshed("h1")
          Wait.waitForElementToPresentByCssSelector(
            ReviewAnswersPage.printthispage
          )
          assert(
            driver
              .findElement(By.cssSelector(ReviewAnswersPage.printthispage))
              .getText
              .contains(linkText)
          )
      }
    ...
    pageNumber match {
      ...
      case "Previous" =>
        assert(
          driver
            .findElement(
              By.cssSelector(TransactionHistorySecondPage.previousPageCTA)
            )
            .isDisplayed
        )
    }
