...
class StepDef extends BaseStepDef {
  ...
  And("""^I should see (.*) link$""") { (linkText: String) =>
    Wait.waitForTagNameToBeRefreshed("h1")
    Wait.waitForElementToPresentByCssSelector(CreateSubErrorPage.taskListPageLink)
  }
